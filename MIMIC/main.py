...
def main():
    ...
    MODE = "test"  # Select "train" or "test", "Resume", "plot", "Threshold"
    ...
    if MODE == "plot":
        ...
        for i in range(len(factor)):
            ...
            # plot_TPR_MIMIC(pred, diseases, factor[i], factor_str[i])
    ...
    # if MODE == "mean":
    #     pred = pd.read_csv("./results/bipred.csv")
    #     factor = [gender, age_decile, race, insurance, marital_status]
    #     factor_str = ['gender', 'age_decile', 'race', 'insurance', 'marital_status']
    #     for i in range(len(factor)):
    #         mean(pred, diseases, factor[i], factor_str[i])

    # if MODE == "plot_14":
    #     pred = pd.read_csv("./results/bipred.csv")
    #     factor = [age_decile, race, insurance, marital_status]
    #     factor_str = ['age_decile', 'race', 'insurance', 'marital_status']
    #     for i in range(len(factor)):
    #         #   plot_14(pred, diseases, factor[i], factor_str[i])
